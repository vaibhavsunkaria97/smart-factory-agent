    ...
    for i in range(rows):
        ...
        if np.random.rand() < 0.12:
            ...
            if 'temp' in abnormal_channels:
                temp = np.random.uniform(43, 45) if np.random.rand() < 0.5 else np.random.uniform(52, 55)
            if 'pressure' in abnormal_channels:
                pressure = np.random.uniform(0.97, 1.00) if np.random.rand() < 0.5 else np.random.uniform(1.08, 1.10)
            if 'vibration' in abnormal_channels:
                vibration = np.random.uniform(0.07, 0.10)
